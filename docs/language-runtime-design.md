...
| Память демонстрационного компьютера и команда | Пользователь указал 32 ГБ RAM и 7 участников: 2 — визуализация/брокер, 3 — язык/компилятор, 1 — ВМ, 1 — симуляция. CPU, ОС и срок пока не известны; рекомендуемые роли — в [распределении команды](team-allocation.md) |
