...
Подтверждено пользователем: команда из семи человек, 32 ГБ RAM; сети Кана служат ориентиром; 3D-представление метрик в форме торнадо пока отложено. CPU, ОС и срок неизвестны, поэтому ниже указаны зависимости и результаты этапов без календарного обещания. Предложенное распределение участников и порядок совместной работы описаны в [team-allocation.md](team-allocation.md).
...
В команде семь человек. Два участника выбрали визуализацию/брокер, три — язык/компилятор, один — ВМ, один — симуляцию. Предлагается следующее распределение; номера пока не привязаны к именам.

| Участник | Зона ответственности | Основной интерфейс |
|---|---|---|
| №1 — визуализация | Дашборд, метрики, инспектор дома, представление отчётов; опциональная карта | Подтверждённые snapshots и telemetry |
| №2 — брокер и супервизор | Протокол, процессы, модельные часы, барьер тика, нагрузочные замеры; координация общего запуска | Frame/Result, lifecycle, вызов WorldStep |
| №3 — синтаксис | ANTLR4, парсер, AST, синтаксические ошибки; позднее помощь с генератором поселения | AST с координатами исходного текста |
| №4 — семантика | Имена, типы, единицы, события/эффекты и IR | Типизированное промежуточное представление |
| №5 — codegen и поведение | Байт-код, source map, дизассемблер; после первого прохода DSL-программы и сценарии | ISA, `.cvm`, стандартные события и намерения |
| №6 — ВМ | Loader/verifier, стековый интерпретатор, память, лимиты, intrinsics, клиент брокера | ISA, Frame/Result, диагностика исполнения |
| №7 — симуляция | Физический мир, сети, ресурсы, движение, повреждения, конфликты и экономика | View, intents, события результата, snapshots |

Подробные задачи, рабочие пары и переходы между этапами — в [распределении команды](team-allocation.md). №5 помогает №7 программами поведения и сценариями, а №3 после стабилизации грамматики — генератором поселения. Это уменьшает нагрузку на одного владельца физического ядра. №2 рекомендуется как интегратор общего запуска; владельцы компонентов поддерживают собственные интеграции.

Сначала один вертикальный срез собирается совместно; затем компилятор, ядро и UI развиваются параллельно по согласованным схемам. Первый общий запуск должен появиться до завершения всего языка и всех физических моделей.
